    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            if options["deactivate_missing"]:
                # Field-created assets are never in the spreadsheet, so
                # they must be excluded or this would switch off every
                # asset the engineers added on site.
                stale = Asset.objects.filter(
                    is_active=True, is_user_created=False
                ).exclude(structure_code__in=seen_codes)
